...
How It Works
The ultrasonic sensor continuously measures distance to the gate and classifies it into a zone:
...
When an RFID card is scanned, its UID is compared against the stored authorized UID.
If valid, the LCD shows "Access Granted," the buzzer stops, and the servo opens the gate for a few seconds before auto-closing.
If invalid, the LCD shows "Access Denied" and the gate stays locked.
Components Used
Component	Quantity
...
Open the .ino sketch and update authorizedUID with your own RFID card's UID (printed to Serial Monitor when you scan a card).
Upload the sketch to the Maker UNO / Arduino Uno.
Open the Serial Monitor (9600 baud) to view live status logs.
Limitations
